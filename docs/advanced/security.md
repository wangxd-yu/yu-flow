...
已发布 API 的可选入站兜底（鉴权 / 防重放 / 限流 / IP）见 [入站防护与宿主网关分工](./ingress-security.md)。

系统 Redis Key 前缀与清单见 [Redis Key 命名规范](./redis-key-convention.md)。
...
# 恢复默认（走 yml）。若历史上写过旧键，一并删掉
DEL flow:ui:enable
DEL flow:uiEnable
```
最迟不超过 5 秒，拦截器即会在分布式集群所有节点上同步生效并阻断对管理页面的 403 越权访问。无缓存值时，平台自动平滑降级至 `application.yml` 的 `yu.flow.enabled` / `enable-ui` 静态配置。旧键 `flow:uiEnable` 仍可读，但请改用 `flow:ui:enable`。
