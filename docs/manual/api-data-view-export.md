...
- **签名**：token 为 `payload.hmac` 结构，HMAC-SHA256（密钥复用管理端 JWT 密钥）；密钥缺失或仍为历史默认值时**拒绝签发**。签名比较采用常量时间算法，防时序侧信道。
- **TTL**：签发时强制收敛在 **30～3600 秒**（默认 300）；过期后返回 401。
- **非一次性**：有效期内**可重复下载**（兼容浏览器多次请求 / 下载工具续传）。请保持短 TTL，勿将带 token 的 URL 落入日志、工单或对外分享渠道。
- **失效方式**：修改 `YU_FLOW_JWT_SECRET`（全部存量链立即失效）、关闭接口 `signedLinkEnabled`、或下线/取消发布接口。
- **参数固化**：查询 / 路径 / Body 参数在签发时固化进 payload，下载方无法篡改查询范围；强制按已发布快照导出，不受草稿影响。
- 上述语义由单测 `ExcelExportLinkTokenTest` 锁定（TTL 过期、签名篡改、弱默认密钥拒签、payload 往返、签名熵）。
