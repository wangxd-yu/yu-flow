...
/** 健康度枚举值 */
export type HealthStatus = 'HEALTHY' | 'UNHEALTHY' | 'UNKNOWN' | 'CIRCUIT_OPEN';

/** SQL 安全墙（Druid Wall）产品化配置 */
export interface DataSourceWallConfig {
  enabled?: boolean;
  multiStatementAllow?: boolean;
  commentAllow?: boolean;
  noneBaseStatementAllow?: boolean;
  selectAllow?: boolean;
  insertAllow?: boolean;
  updateAllow?: boolean;
  deleteAllow?: boolean;
  ...
  /** 只读表：允许 SELECT，禁止 INSERT/UPDATE/DELETE */
  tableReadOnlyList?: string[];
...
export interface DataSourceDO {
  ...
  wallConfig?: DataSourceWallConfig;
  createTime?: string;
  updateTime?: string;
  // ---------- 健康度追踪字段 ----------
