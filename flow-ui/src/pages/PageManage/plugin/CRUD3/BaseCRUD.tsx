...
import { CRUD2Schema as CRUD3Schema } from 'amis';
...
import type { IFormItemStore, IFormStore } from 'amis';
import type { CRUDScaffoldConfig } from 'amis-editor';
import type {
  ScaffoldForm,
...
/** 需要动态控制的属性 */
export type CRUD3DynamicControls = Partial<
  Record<
    DSFeatureType | 'columns' | 'filters' | 'toolbar' | 'primaryField',
    (context: BuildPanelEventContext) => any
  >
>;
export class BaseCRUDPlugin extends BasePlugin {
  static id = 'CRUD3Plugin';
  ...
  $schema = '/schemas/AMISCRUD3Schema.json';
  ...
  get scaffoldForm(): ScaffoldForm {
    return {
      ...
      pipeOut: async (config: CRUDScaffoldConfig) => {
        ...
        if (config.operators?.includes('View') && (config as any).detailMode === 'api' && (config as any).detailApi) {
          const viewAction = findObj(schema, (item: any) => item?.actionType === 'dialog' && item?.dialog?.title === '详情');
          if (viewAction && viewAction.dialog && viewAction.dialog.body) {
            if (viewAction.dialog.body.type === 'form' || viewAction.dialog.body.type === 'service') {
              viewAction.dialog.body.initApi = (config as any).detailApi;
            }
          }
        }
                    ...
                    ? [
                      {
                        ...
                        name: 'detailMode',
                        options: [
                          { label: '表单带出', value: 'static' },
                          { label: '接口查询', value: 'api' }
                        ],
                        value: 'static'
                      },
                      {
                        ...
                        label: '初始化接口',
                        name: 'detailApi',
                        placeholder: '请输入接口地址，如 /api/detail?\\${id}',
                        visibleOn: 'data.detailMode === "api"'
                      }
                    ]
  ...
  protected _dynamicControls: CRUD3DynamicControls = {
    /** 列配置 */
  ...
  set dynamicControls(controls: CRUD3DynamicControls) {
    if (!isObject(controls)) {
      throw new Error(
        '[amis-editor][CRUD3Plugin] dynamicControls的值必须是一个对象'
      );
    }
    ...
    const generateDSControls = () => {
      ...
      const dsSettings = this.dsManager.buildCollectionFromBuilders(
        (builder, builderKey) => {
          return {
            type: 'container',
            visibleOn: `this.dsType == null ? '${builderKey}' === '${defaultDsType || ApiDSBuilderKey
  ...
  renderToolbarCollapse = (context: any) => {
    const builder = this.dsManager.getBuilderBySchema(context.node.schema);
  ...
  renderFiltersCollapse = (context: any) => {
    const builder = this.dsManager.getBuilderBySchema(context.node.schema);
