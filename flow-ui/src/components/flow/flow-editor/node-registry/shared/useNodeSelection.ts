...
export const NODE_HEADER_WITH_ID_HEIGHT = 46;
...
export const NodeHeader: React.FC<NodeHeaderProps> = ({
    ...
    const resolvedHeight = height ?? (nodeId != null ? NODE_HEADER_WITH_ID_HEIGHT : 34);
    ...
    return React.createElement('div', {
        style: {
            ...
            // 由主题浅色向卡片体渐隐，弱化「厚色条」的分区感，更现代
            background: `linear-gradient(180deg, ${t.headerBg} 0%, ${t.headerBg} 46%, #ffffff 100%)`,
            display: 'flex',
            alignItems: 'center',
            padding: '0 10px',
            // 轻分割，避免厚色条「分区感」
        ...
        // 图标统一放入白色圆角小片：抹平各节点 SVG 尺寸差异，并在浅色头部更醒目
...
export const NodeWrapper: React.FC<NodeWrapperProps> = ({
    ...
    // 选中态是 attr(strokeWidth) 驱动，X6 节点 DOM 无 selected class；
    // 手动在节点容器上打 yf-node-selected，供全局端口样式（放大）挂钩。
    React.useEffect(() => {
        const view = graph?.findViewByCell?.(node);
        const el = view?.container as HTMLElement | undefined;
        if (el) el.classList.toggle('yf-node-selected', !!selected);
    }, [graph, node, selected]);
...
export const NODE_FOOTER_HEIGHT = 38;
/**
...
export const NODE_FOOTER_PORT_OFFSET_Y = 19;
...
// Postman 向：右下角斜向抓握纹（发丝线，默认淡、hover 加深，由全局样式控制透明度）
const RESIZE_ICON = React.createElement('svg',
    { viewBox: '0 0 12 12', width: 15, height: 15, fill: 'none', stroke: 'currentColor', strokeWidth: 1.3, strokeLinecap: 'round' },
    React.createElement('path', { d: 'M11 5 L5 11' }),
    React.createElement('path', { d: 'M11 8.5 L8.5 11' }),
);
