...
export const DATABASE_LAYOUT = {
    headerHeight: HEADER_HEIGHT, footerHeight: NODE_FOOTER_HEIGHT, width: MIN_WIDTH,
    payloadPortY: PAYLOAD_PORT_Y,
    get totalHeight() { return HEADER_HEIGHT + DATASOURCE_ROW_HEIGHT + ROW_HEIGHT + VAR_PADDING + MIN_QUERY_HEIGHT + COND_PADDING + NODE_FOOTER_HEIGHT; },
    get footerTop() { return this.totalHeight - NODE_FOOTER_HEIGHT; },
    get outPortY() { return singleOutPortY(this.totalHeight); },
};
...
export const DatabaseNode = ({ node }: { node: Node }) => {
    ...
    React.useEffect(() => {
        import('@/services/flow/dataSource').then(({ queryDataSourceList }) => {
    ...
    const contentH = HEADER_HEIGHT + DATASOURCE_ROW_HEIGHT + variables.length * ROW_HEIGHT + VAR_PADDING + COND_PADDING + NODE_FOOTER_HEIGHT;
