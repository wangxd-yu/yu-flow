...
import { GRAPH_API_METHOD_KEY, methodHasBody } from './flow-editor/node-registry/nodes/request/RequestNodeComponent';
import { debugRunAutoApiConfig, startDebugSession, getDebugSessionStatus, resumeDebugSession, cancelDebugSession } from '@/services/flow/flowController';
...
export default function FlowEditor(props: ExtendedFlowEditorProps) {
    ...
    // ── 外层接口 Method → Request 节点（同步 data.method / Body 端口）──
    React.useEffect(() => {
        const graph = graphRef.current;
        ...
        graph.getNodes().forEach((node) => {
            ...
            // 确保 Body 端口与 method 一致（节点组件也会同步；此处兜底导出 DSL）
