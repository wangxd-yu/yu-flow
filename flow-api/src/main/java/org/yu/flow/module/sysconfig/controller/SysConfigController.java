...
public class SysConfigController {
    ...
    public R<String> getConfigByKey(@PathVariable String configKey) {
        String raw = sysConfigCacheManager.getStringConfig(configKey, "/");
        return R.ok(SysConfigDTO.maskIfSecret(configKey, raw));
    }
    ...
    public R<SysConfigDTO> create(@Valid @RequestBody SaveSysConfigDTO dto) {
        return R.ok(SysConfigDTO.fromDO(sysConfigService.create(dto)));
    }

    @PutMapping("/{id}")
    @RequirePerm("sys:config:write")
    public R<SysConfigDTO> update(@PathVariable String id, @RequestBody SaveSysConfigDTO dto) {
        return R.ok(SysConfigDTO.fromDO(sysConfigService.update(id, dto)));
    }
