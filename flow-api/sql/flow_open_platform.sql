...
CREATE TABLE IF NOT EXISTS `flow_open_platform` (
    ...
    `ip_allowlist`  VARCHAR(1024)         COMMENT 'IP白名单JSON数组，空=不限；支持单 IP 或 CIDR',
    `expire_at`     DATETIME              COMMENT '平台到期时间',
    `open_call_log_enabled` TINYINT NULL DEFAULT 1 COMMENT '是否记录入站摘要日志 0关1开',
    `rate_limit_qps` INT NULL COMMENT '平台级QPS上限，空=不限',
